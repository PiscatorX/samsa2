...
opt$input = "/home/drewx/St.Helena.MetaT/Results/MT/samsa2_1606/RefSeq_results/organisms"
...
basename <- tools::file_path_sans_ext(opt$out)
write.csv(data.frame(complete_table), paste0(basename,".data"))
...
graphing_table[,"sample"] <- tools::file_path_sans_ext(basename(rownames(flipped_complete_table)))
...
setwd(cwd)
write.csv(graphing_table, paste0(basename,".metrics"))
